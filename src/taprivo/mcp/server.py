...
from taprivo.mcp.statusline import parse_payload, status_text
...
def _agent_out(status: AgentStatus) -> AgentOut:
    age_ms = max(int(time.time() * 1000) - status.received_at_ms, 0)
    return AgentOut(
        model=status.model,
        context_used=status.context_used,
        cost_usd=status.cost_usd,
        duration_s=status.duration_s,
        five_hour_used=status.five_hour_used,
        five_hour_resets_at=status.five_hour_resets_at,
        seven_day_used=status.seven_day_used,
...
def build_server(engine: EnergyEngine, config: Config) -> MCPServer:
    ...
    def get_stats(scope: Literal["session"] = "session") -> StatsOut:
        ...
        return StatsOut(
            ...
            bpm=s.bpm,
            rhythm_steady=s.rhythm_steady,
            rhythm_multiplier=s.rhythm_multiplier,
    ...
    def get_session() -> SessionOut:
        ...
        return SessionOut(
            ...
            agent=_agent_out(s.agent) if s.agent else None,
        )

    return mcp


def statusline_route(engine: EnergyEngine) -> Route:
    """Claude Code's status line payload in, Taprivo's energy segment out.

    The bearer token, Host, Origin, body-size and rate limits are enforced by
    LocalGuardMiddleware around the whole app, exactly as they are for /mcp.
    The body is parsed into an AgentStatus and dropped: it is never logged and
    never reaches the statistics store.
    """
    ...
    async def statusline(request: Request) -> Response:
        ...
        engine.set_agent_status(status)
        return PlainTextResponse(status_text(engine.snapshot(), status))

    return Route(STATUSLINE_PATH, endpoint=statusline, methods=["POST"])
...
def build_asgi_app(engine: EnergyEngine, config: Config, token: str) -> ASGIApp:
    ...
    inner.routes.append(statusline_route(engine))
